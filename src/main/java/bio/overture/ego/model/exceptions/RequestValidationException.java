  ...
  public static <T> void checkRequestValid(@NonNull T objectToValidate) {
    ...
    if (!errors.isEmpty()) {
      val requestViolations =
          errors.stream().map(RequestViolation::createRequestViolation).collect(toImmutableSet());
      val formattedMessage =
          "The object of type '%s' with value '%s' has the following constraint violations: [%s]";
