...
public class FacebookTokenService {
  ...
  private static final String USER_GIVEN_NAME = "given_name";
  ...
  */
  ...
  public boolean validToken(String fbToken) {
    ...
    try {
      return fbConnector.execute(
          new URI(tokenCheckUri),
          HttpMethod.GET,
          null,
          response -> {
  ...
  public Optional<IDToken> getAuthInfo(String fbToken) {
    ...
    try {
      ...
      log.debug(
          "Error getting email response from Facebook for uri: {}, {}",
          userDetailsUri,
          ex.getMessage());
      return Optional.empty();
    }
