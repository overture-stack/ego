...
public class UserPermissionService extends AbstractPermissionService<User, UserPermission> {
  ...
  /**
   * Decorates the call to addPermissions with the functionality to also cleanup user tokens in the event
   * that the permission added downgrades the available scopes to the user.
   ...
   */
  @Override
  public User addPermissions(@NonNull UUID userId, @NonNull List<PermissionRequest> permissionRequests) {
    val user = super.addPermissions(userId, permissionRequests);
    tokenService.cleanupTokens(ImmutableSet.of(userService.getById(userId)));
    return user;
  }
