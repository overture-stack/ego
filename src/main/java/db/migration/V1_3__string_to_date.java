...
public class V1_3__string_to_date implements SpringJdbcMigration {
  ...
  public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
    ...
    jdbcTemplate.execute(
        "ALTER TABLE EGOUSER ALTER CREATEDAT DROP DEFAULT, ALTER CREATEDAT TYPE TIMESTAMP WITHOUT TIME ZONE USING DATE(CREATEDAT);");

    jdbcTemplate.execute(
        "ALTER TABLE EGOUSER ALTER LASTLOGIN DROP DEFAULT, ALTER LASTLOGIN TYPE TIMESTAMP WITHOUT TIME ZONE USING DATE(LASTLOGIN);");
  ...
  private void createTestData(JdbcTemplate jdbcTemplate, UUID userOneId, UUID userTwoId) {
    jdbcTemplate.update(
        "INSERT INTO EGOUSER (id, name, email, role, status, createdAt, lastlogin) "
            + "VALUES (?, 'userOne', 'userOne@email.com', 'user', 'Pending', '2017-01-15 04:35:55', '2016-12-15 23:20:51')",
        userOneId);

    jdbcTemplate.update(
        "INSERT INTO EGOUSER (id, name, email, role, status, createdAt, lastlogin) "
            + "VALUES (?, 'userTwo', 'userTwo@email.com', 'user', 'Pending', '2017-04-05 05:05:50', '2017-06-16 02:44:19')",
        userTwoId);
  }

  private void testDateType(JdbcTemplate jdbcTemplate) {
    val egoUsers =
        jdbcTemplate.query("SELECT * FROM EGOUSER", new BeanPropertyRowMapper(User.class));
